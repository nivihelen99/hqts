...
protected:
    FlowTable flow_table; // Instance of std::unordered_map<core::FlowId, core::FlowContext>
};

TEST_F(FlowTableTest, EmptyTable) {
    ASSERT_TRUE(flow_table.empty());
    ASSERT_EQ(flow_table.size(), 0);
}
...
TEST_F(FlowTableTest, AddAndFindFlow) {
    ...
    // Add using operator[]
    flow_table[id1] = context1;
    ASSERT_EQ(flow_table.size(), 1);
    ASSERT_FALSE(flow_table.empty());

    // Find using find()
    auto it = flow_table.find(id1);
    ASSERT_NE(it, flow_table.end());
    ...
    auto it_non_existent = flow_table.find(id_non_existent);
    ASSERT_EQ(it_non_existent, flow_table.end());
}

TEST_F(FlowTableTest, AddMultipleFlows) {
    core::FlowId id1 = 1;
    core::FlowId id2 = 2;
    flow_table[id1] = createTestFlowContext(id1);
    flow_table[id2] = createTestFlowContext(id2, 202);

    ASSERT_EQ(flow_table.size(), 2);

    auto it1 = flow_table.find(id1);
    ASSERT_NE(it1, flow_table.end());
    ASSERT_EQ(it1->second.flow_id, id1);

    auto it2 = flow_table.find(id2);
    ASSERT_NE(it2, flow_table.end());
...
TEST_F(FlowTableTest, UpdateFlowContext) {
    core::FlowId id1 = 100;
    flow_table[id1] = createTestFlowContext(id1, 10, 1, core::DropPolicy::TAIL_DROP);
    ...
    // Verify update
    auto it_updated = flow_table.find(id1);
    ASSERT_NE(it_updated, flow_table.end());
    ...
    flow_table[id1] = new_context; // Replaces existing or inserts if not present

    it_updated = flow_table.find(id1);
    ASSERT_NE(it_updated, flow_table.end());
...
TEST_F(FlowTableTest, EraseFlow) {
    ...
    core::FlowId id2 = 2;
    flow_table[id1] = createTestFlowContext(id1);
    flow_table[id2] = createTestFlowContext(id2);
    ASSERT_EQ(flow_table.size(), 2);

    // Erase by key
    size_t erased_count = flow_table.erase(id1);
    ASSERT_EQ(erased_count, 1);
    ASSERT_EQ(flow_table.size(), 1);
    ASSERT_EQ(flow_table.find(id1), flow_table.end()); // Verify it's gone
    ASSERT_NE(flow_table.find(id2), flow_table.end()); // Verify other is still there

    // Try erasing non-existent key
    erased_count = flow_table.erase(999);
...
TEST_F(FlowTableTest, AddUsingInsert) {
    ...
    auto result = flow_table.insert({id1, context1});
    ASSERT_TRUE(result.second); // bool part of pair is true if insertion took place
    ASSERT_EQ(flow_table.size(), 1);
    ASSERT_NE(result.first, flow_table.end()); // iterator should be valid
    ...
    auto result_dup = flow_table.insert({id1, context2_dup});
    ASSERT_FALSE(result_dup.second); // Insertion should not happen for duplicate key
    ASSERT_EQ(flow_table.size(), 1); // Size should remain 1
    ASSERT_NE(result_dup.first, flow_table.end()); // iterator should point to existing element
